...
app.use(cors({
    origin: "mongodb+srv://supreetha1512:supree1512@cluster0.qau4u.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0", // Adjust based on your frontend URL
...
app.get("/recipes/details/:id", async (req, res) => {
    try {
        const url = `https://api.spoonacular.com/recipes/${req.params.id}/information?apiKey=${process.env.RECIPE_API_KEY}`;
        const response = await axios.get(url);
        const recipe = {
